...
backup_conf(){
	dbus list merlinclash_device_ |  sed 's/=/=\"/' | sed 's/$/\"/g'|sed 's/^/dbus set /' | sed '1 isource /koolshare/scripts/base.sh' |sed '1 i#!/bin/sh' > /tmp/upload/clash_passdevicebackup.sh
}

remove_silent(){
	echo_date 先清除已有的参数... >> $LOG_FILE
	devices=`dbus list merlinclash_device_ | cut -d "=" -f 1`
