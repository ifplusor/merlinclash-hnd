...
echo_date "机型：$MODEL $(show_firmware_type)"
...
echo_date "当前jffs分区剩余 ${SPACE_AVAL}KB, 插件安装需要 ${SPACE_NEED}KB"

if [ "$SPACE_AVAL" -lt "$SPACE_NEED" ]; then
	exit_install 2
fi

echo_date "符合安装要求，开始安装插件！"
...
find /koolshare/init.d/ -name "*clash.sh" -exec rm -rf {} +
...
if nvram get extendno | grep -q koolshare && [ "$(nvram get productid)" == "RT-AC86U" ] && [ "${EXT_NU}" -lt "81918" ]; then
	ROG_86U=1
fi

# 判断固件需要什么UI
if [ "$MODEL" == "GT-AC5300" ] || [ "$MODEL" == "GT-AX11000" ] || [ "$ROG_86U" == "1" ]; then
	# 官改固件，骚红皮肤
	ROG=1
fi

mkdir -p /koolshare/bin/Music
mkdir -p /koolshare/merlinclash/dashboard
mkdir -p /koolshare/merlinclash/yaml_basic
mkdir -p /koolshare/merlinclash/yaml_dns
mkdir -p /koolshare/merlinclash/yaml_bak
mkdir -p /koolshare/merlinclash/yaml_use
mkdir -p /koolshare/merlinclash/subconverter
...
# chmod 644 /koolshare/merlinclash/Country.mmdb
# chmod 644 /koolshare/merlinclash/yaml_basic/*
# chmod 644 /koolshare/merlinclash/yaml_dns/*
# chmod 644 /koolshare/merlinclash/subconverter/*
chmod 755 /koolshare/merlinclash/subconverter/subconverter
# chmod 644 /koolshare/merlinclash/*
...
if [ "$merlinclash_enable" == "1" ]; then
	echo_date "重启clash插件！"
	sh /koolshare/scripts/clashconfig.sh restart
fi
